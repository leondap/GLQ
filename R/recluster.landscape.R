recluster.landscape<-function (mat, units = NULL, dist, transcorr, map_alt, map1, minsea = 2) 
{
    ...
    for (i in 1:nrow(try$delsgs)) {
	  ...
	  pa <- as(patch, "sf")
        nume <- as.numeric(round(st_length(pa) * 0.003, 0))
        ...
        if (nume > 1) {
            ...
            midpointx[i] <- as.numeric(val[round((lengthpath[i] + 1e-04)/2, 0), 1])
            midpointy[i] <- as.numeric(val[round((lengthpath[i] + 1e-04)/2, 0), 2])
    ...
    table <- cbind(try$delsgs, gendist, midpointx, midpointy, 
        lengthpath, seapointx, seapointy, lengthsea, altimin, 
        altimax)
    return(table)
}
